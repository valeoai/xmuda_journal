...
def train(cfg, output_dir='', run_name=''):
    ...
    train_dataloader_trg_labeled = build_dataloader(cfg, mode='train_labeled', domain='target',
                                                    start_iteration=start_iteration,
                                                    different_batch_size=trg_batch_size)
    train_dataloader_trg_unlabeled = build_dataloader(cfg, mode='train_unlabeled', domain='target',
                                                      start_iteration=start_iteration)
    ...
    train_iter_trg_labeled = enumerate(train_dataloader_trg_labeled)
    train_iter_trg_unlabeled = enumerate(train_dataloader_trg_unlabeled)
    for iteration in range(start_iteration, max_iteration):
        ...
        _, data_batch_trg_labeled = train_iter_trg_labeled.__next__()
        _, data_batch_trg_unlabeled = train_iter_trg_unlabeled.__next__()
        data_time = time.time() - end

        # collate src + trg_labeled batch
        batch_size = data_batch_src['img'].shape[0]
        locs_trg = data_batch_trg_labeled['x'][0].clone()
        locs_trg[:, 3] += batch_size
        x_collate = [torch.cat([data_batch_src['x'][0], locs_trg], dim=0),
                     torch.cat([data_batch_src['x'][1], data_batch_trg_labeled['x'][1]], dim=0)]
        img_indices_concat = data_batch_src['img_indices'] + data_batch_trg_labeled['img_indices']
        data_batch_collate = {
            'x': x_collate,
            'seg_label': torch.cat([data_batch_src['seg_label'], data_batch_trg_labeled['seg_label']], dim=0),
            'img': torch.cat([data_batch_src['img'], data_batch_trg_labeled['img']], dim=0),
        ...
        if 'SCN' in cfg.DATASET_SOURCE.TYPE and 'SCN' in cfg.DATASET_TARGET.TYPE:
            ...
            # target unlabeled
            data_batch_trg_unlabeled['x'][1] = data_batch_trg_unlabeled['x'][1].cuda()
            data_batch_trg_unlabeled['img'] = data_batch_trg_unlabeled['img'].cuda()
            if cfg.TRAIN.XMUDA.lambda_pl > 0:
                data_batch_trg_unlabeled['pseudo_label_2d'] = data_batch_trg_unlabeled['pseudo_label_2d'].cuda()
                data_batch_trg_unlabeled['pseudo_label_3d'] = data_batch_trg_unlabeled['pseudo_label_3d'].cuda()
        else:
        ...
        preds_2d = model_2d(data_batch_trg_unlabeled)
        preds_3d = model_3d(data_batch_trg_unlabeled)
        ...
        if cfg.TRAIN.XMUDA.lambda_pl > 0:
            ...
            pl_loss_trg_2d = F.cross_entropy(preds_2d['seg_logit'], data_batch_trg_unlabeled['pseudo_label_2d'],
                                             weight=class_weights_trg)
            pl_loss_trg_3d = F.cross_entropy(preds_3d['seg_logit'], data_batch_trg_unlabeled['pseudo_label_3d'],
                                             weight=class_weights_trg)
...
def main():
    ...
    # this script uses DATASET_TARGET.TRAIN_LABELED and DATASET_TARGET.TRAIN_UNLABELED
    assert not cfg.DATASET_TARGET.TRAIN
